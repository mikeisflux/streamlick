...
export function GuestGreenroom({
  ...
  useEffect(() => {
    if (videoRef.current && broadcastStream) {
      if (videoRef.current.srcObject !== broadcastStream) {
        videoRef.current.srcObject = broadcastStream;
        videoRef.current.volume = streamVolume;
        videoRef.current.play().catch((err) => console.warn('[GuestStage] Play failed:', err));
      }
    }
  ...
  useEffect(() => {
    if (selfVideoRef.current && localStream) {
      if (selfVideoRef.current.srcObject !== localStream) {
  ...
  // When LIVE, show the StreamYard-style stage view
  if (status === 'live') {
    return (
            ...
            <div className="relative bg-white rounded-xl shadow-2xl w-full max-w-md mx-4 p-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 p-3 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    <h3 className="font-medium text-gray-900 text-sm">Screen share not working?</h3>
                    <p className="text-xs text-gray-500">Make sure you've granted screen recording permissions in your browser.</p>
                  </div>
                </div>
