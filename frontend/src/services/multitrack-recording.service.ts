...
class MultiTrackRecordingService {
  ...
  async addParticipantTrack(
    ...
    if (this.config.separateAudioVideo) {
      ...
      if (audioTracks.length > 0) {
        ...
        const audioRecorder = this.createMediaRecorder(audioStream, 'audio');
        tracks.push({
          participantId,
          participantName,
      ...
      if (videoTracks.length > 0) {
        ...
        const videoRecorder = this.createMediaRecorder(videoStream, 'video');
        tracks.push({
          participantId,
          participantName,
      ...
      const combinedRecorder = this.createMediaRecorder(stream, 'both');
      tracks.push({
        participantId,
        participantName,
        mediaRecorder: combinedRecorder,
        chunks: [],
        stream,
        trackType: 'both',
      });
  ...
  private createMediaRecorder(
    ...
    // Store chunks reference for later access
    const trackEntry = Array.from(this.participantTracks.values())
      .flat()
      .find((t) => t.mediaRecorder === null);

    if (trackEntry) {
      trackEntry.chunks = chunks;
    }
